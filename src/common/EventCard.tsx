...
import { getCategoryIcon, getCategoryLabel } from "@/src/utils/constants";
...
export default function EventCard({
  ...
  if (variant === "list") {
    return (
      <Card
        className={`py-0 group cursor-pointer  hover:shadow-lg transition-all hover:border-green-500/50 ${className}`}
        onClick={onClick}
      >
  ...
  return (
    <Card
      className={`overflow-hidden group pt-0 ${onClick ? "cursor-pointer hover:shadow-lg transition-all hover:border-green-500/50" : ""} ${className}`}
      onClick={onClick}
    >
      ...
      <CardContent className="space-y-3">
        <div>
          ...
          <h3 className="font-semibold text-lg line-clamp-2 group-hover:text-green-400 transition-colors">
            {event.title}
          </h3>
        </div>
